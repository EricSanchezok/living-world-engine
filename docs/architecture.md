...
The diagnostic [plan and transition co-generation](specs/0179-fuse-reviewed-plan-and-transition-generation.md) binds a provisional transition to complete, independently reviewed initial plans. Only unchanged first-pass plans with no check or additional randomness can reuse it. The original transition materializer, mechanics, causal review and commit retain authority.
...
3. New actions are grounded before boundary selection. Their footprint and resource-pool keys are queried against an ephemeral exact Activity index; interruptibility and positive remaining duration determine candidate onset pairs. The [observer-bound receipt contract](specs/0128-observer-bound-onset-receipts.md) adjudicates their private stimuli and binds the resulting frozen reaction set.
...
9. Observation Renderer fills engine-owned observer slots for normal, blocked, queued, reserved, started, and contested outcomes. [Observed external-event evidence](specs/0165-observed-activity-interruptions.md) controls subsequent observation interruptions and is recomputed after observation repair before final review. Only Agents at a new decision point run AgentMind, and external reaction requests pause at a persisted `WorldStepPreparation` rather than keeping an execution open.
...
The benchmark-only [explicit compilation access sets](specs/0175-explicit-compilation-access-sets.md) rename the two dependency arrays on the wire and restore them before original validation. This representation neither infers dependencies nor changes the concurrency or commit contract.
