...
Keep complete global planning requests within the existing model input limit by sharing repeated source evidence and ordered action menus. Full player diagnostics reach a global fallback whose complete source inventory expands for every action and is copied into the planning worklist. The user authorizes this bounded representation experiment under the continuing nonthinking optimization work.
...
Measure exact round-trip equality and request bytes on the complete frozen pre-replacement preparation, labeled as a controlled construction rather than an exact replay of the later failed global request. Freeze one pooled global source viability call after checked code is committed. Record the complete request, output, HTTP count, tokens, latency and first-pass admission. An oversized control cannot be dispatched and supplies no paired latency comparison. A form pass only permits semantic review and an independently frozen complete player diagnostic.
...
[Adapter](../../src/engine/benchmarks/step-efficiency/global-means-pool.ts), [full player entry](../../scripts/experiments/player-integrated-playtest.ts), and [representation decision](../decisions/0199-share-global-planning-source-inventories.md) own implementation, execution and rationale respectively.
