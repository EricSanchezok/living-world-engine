...
The [compilation comparison](../../scripts/experiments/player-compilation-comparison.ts) preserves the original state order and permits an arrival revision before the first action: step, elapsed time and committed action history establish that boundary. The counterfactual adds the declared mechanics to truth and its replay base and reverses them exactly to the source. Both arms use independent cold query caches per physical batch and warm passage caches, matching the five recorded first retrievals. Baseline context, shortlist, ordered HTTP request, normalization and slot rejection must reproduce historical evidence before network access. Candidate preparation captures its request without replaying baseline aliases against a changed dictionary. A clean committed producer must match the frozen manifest at live dispatch; all new response usage must reconcile with the single physical gateway audit before another call.
