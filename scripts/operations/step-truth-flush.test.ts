...
async function run(postPromise: boolean) {
  const selected = stepEfficiencyAlgorithmRef({ sourceInventory: true, resolutionRepresentation: "resolution-dependent-fields-v1",
    truthTransport: "shared-state-first-v1", ...(postPromise ? { truthFlushBoundary: "post-promise-v1" } : {}) });
  expect(registerBuiltinAlgorithms().has(selected)).toBe(true);
  // Isolate the exact selected batching child through the registered ordered
  // engine; deterministic full-catalog siblings do not qualify model semantics.
  ...
  const transitions = physical.filter(request => request.role === "truth-transition");
  const contexts = transitions.flatMap(request => {
    const state = (request.context as { state?: unknown }).state;
    return isSharedBatchContext(state) ? expandSharedBatchContexts(state) : [request.context];
  });
  expect(contentHash(replaySimulationState(result.state).truth)).toBe(contentHash(result.state.truth));
  return { transitions, contexts, result };
}

it("coalesces ordered transitions through the selected registered batching child while preserving world effects", async () => {
  ...
  expect(after.transitions[0]!.schemaName).toBe("truth_transition_batch");
  ...
  expect(() => stepEfficiencyAlgorithmRef({ truthFlushBoundary: "post-promise-v1" })).toThrow("reviewed shared transport");
});
