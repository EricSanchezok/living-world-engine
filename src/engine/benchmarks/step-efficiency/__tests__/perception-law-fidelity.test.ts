...
import { verifyOnlyPerceptionSourceFidelityAdded } from "../perception-source-fidelity";
import { perceptionLawContextRequest } from "../perception-law-context";
import { perceptionLawFidelityRequest } from "../perception-law-fidelity";

it.each(["done", "check", "wrong-owner", "repair"])("isolates added fidelity after the complete law index for a real %s directive, preserving original acceptance and RNG", async mode => {
  ...
  const input: OnsetPerceptionInput = { definition, state, identityOwner: "law-fidelity-test", groundings: [],
  ...
  for (const candidate of [false, true]) {
    ...
    const gateway = createModelGateway(catalog, { TEST_MODEL_API_KEY: "test-only" }, { registry: createTestModelRegistry(catalog), maxTransportAttempts: 1,
      fetchForAccount: () => async (_url, init) => {
        ...
        return new Response(JSON.stringify({ id: "law-fidelity-test", model: "scripted:truth-deepseek",
    ...
    const provider: StructuredModelProvider = { catalog, availableProfileSummaries: role => gateway.availableProfileSummaries(role), assertProfilesAvailable: ids => gateway.assertProfilesAvailable(ids),
      generateStructured: request => {
        const adapted = candidate ? perceptionLawFidelityRequest(request) : perceptionLawContextRequest(request);
        ...
        if (candidate) expect(() => perceptionLawFidelityRequest(adapted)).toThrow("unmodified");
        return gateway.generateStructured(adapted);
      } };
    const run = new TruthEngine(provider, { repairAttempts: mode === "repair" ? 1 : 0 }).perceiveOnset(input, { workloadId: "law-fidelity-world", batchId: "law-fidelity-step", runtimeIdentity: { worldHash: state.worldHash, revision: state.revision } });
...
it("leaves unrelated roles alone and refuses a combined layout experiment", () => {
  ...
  expect(perceptionLawFidelityRequest(request)).toBe(request);
  expect(() => perceptionLawFidelityRequest({ ...request, role: "truth-perception", schemaName: "truth_perception_directive" })).toThrow("unmodified");
  expect(() => perceptionLawFidelityRequest({ ...request, role: "truth-perception", schemaName: "truth_perception_directive", jsonExamplePolicy: undefined, jsonObjectPostlude: "Extra source layout" })).toThrow("unmodified");
});
