...
import { perceptionNoExampleRequest, verifyOnlyPerceptionExampleOmitted } from "../perception-example-policy";

it.each(["done", "check", "wrong-owner"])("isolates the physical example for a real %s directive, preserving original acceptance and RNG", async mode => {
  ...
  const input: OnsetPerceptionInput = { definition, state, identityOwner: "example-policy-test", groundings: [],
  ...
  for (const candidate of [false, true]) {
    ...
    const gateway = createModelGateway(catalog, { TEST_MODEL_API_KEY: "test-only" }, { registry: createTestModelRegistry(catalog), maxTransportAttempts: 1,
      fetchForAccount: () => async (_url, init) => {
        ...
        return new Response(JSON.stringify({ id: "example-test", model: "scripted:truth-deepseek",
    ...
    const provider: StructuredModelProvider = { catalog, availableProfileSummaries: role => gateway.availableProfileSummaries(role), assertProfilesAvailable: ids => gateway.assertProfilesAvailable(ids),
      generateStructured: request => {
        const adapted = candidate ? perceptionNoExampleRequest(request) : request;
        ...
        if (candidate) expect(() => perceptionNoExampleRequest(adapted)).toThrow("unmodified");
        return gateway.generateStructured(adapted);
      } };
    const run = new TruthEngine(provider, { repairAttempts: 0 }).perceiveOnset(input, { workloadId: "example-world", batchId: "example-step", runtimeIdentity: { worldHash: state.worldHash, revision: state.revision } });
  ...
  for (let i = 0; i < bodies[0]!.length; i++) {
    expect(() => verifyOnlyPerceptionExampleOmitted(bodies[0]![i], bodies[1]![i])).not.toThrow();
    ...
    expect(() => verifyOnlyPerceptionExampleOmitted(bodies[0]![i], damaged)).toThrow("more than");
  }
});
...
it("leaves unrelated roles alone and refuses a combined layout experiment", () => {
  ...
  expect(perceptionNoExampleRequest(request)).toBe(request);
  expect(() => perceptionNoExampleRequest({ ...request, role: "truth-perception", schemaName: "truth_perception_directive" })).toThrow("unmodified");
  expect(() => perceptionNoExampleRequest({ ...request, role: "truth-perception", schemaName: "truth_perception_directive", jsonExamplePolicy: undefined, jsonObjectPostlude: "Extra source layout" })).toThrow("unmodified");
});
