...
it.each(["continuing", "arrived"] as const)("distinguishes consumed interval receipt from actual %s goal completion", async scenario => {
  let captured = 0;
  const { source, result } = await runConditionalCompletionScenario(scenario, async (request, fallback) => {
    ...
    expect(request.system).toContain("not evidence that the whole action has finished");
    expect(request.system).toContain("including while the Activity continues");
    expect(request.system).toContain("independent evidence that the complete task can end");
